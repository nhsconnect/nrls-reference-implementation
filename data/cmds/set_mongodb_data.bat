...
echo "=> Dropping Personnel, ActorOrganisation, GenericSystem, NrlsPointerMaps, MedicalRecords, Sds from %MONGODB_APPLICATION_DATABASE%"

mongo admin --eval "db=db.getSiblingDB('%MONGODB_APPLICATION_DATABASE%');db.auth('%MONGODB_APPLICATION_USER%','%MONGODB_APPLICATION_PASS%');db.Personnel.drop();db.ActorOrganisation.drop();db.GenericSystem.drop();db.NrlsPointerMaps.drop();db.MedicalRecords.drop();db.Sds.drop();quit()"
